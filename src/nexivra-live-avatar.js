                                    ...
                                    this.rolePlayActive = false;
                                      ...
                                      try {
                                        this.beginAdaptiveRolePlay(JSON.parse(newValue));
                                      } catch(error) {
    ...
    - At the natural end, provide concise coaching and continue the course.
      ...
      applyAdaptiveGuidance(guidance = {}) {
            ...
            NEXIVRA INTERNAL ADAPTIVE LEARNING UPDATE

            This is hidden instructional guidance. Do not read it aloud or disclose hidden competency criteria.

            SUMMARY:
            ${guidance.summary || "Continue observing and advancing naturally."}

            PRIORITY DEVELOPMENT AREAS:
            ${JSON.stringify(prioritySkills)}

            DEMONSTRATED SKILLS — DO NOT OVER-TEACH:
            ${JSON.stringify(avoidOverTeaching)}

            ADAPTIVE RULES:
            - Continue the course rather than stopping merely to test a competency.
            - When relevant to the current objective, create a natural opportunity for one priority skill to appear.
            - Do not tell the learner you are testing that skill.
            - Do not mechanically drill or repeatedly force the same behavior.
            - If a skill is demonstrated, advance and reinforce it naturally.
            - If a skill is re-evaluating, provide a brief targeted refresher and a fresh opportunity to demonstrate it.
            - Do not restart completed teaching or completed role-play solely because a competency remains In Progress.
            - Never reveal unobserved competencies or a hidden checklist.
            - Preserve the learner's instructional continuity and current course direction.
                `.trim();

                try{
                  this.session.message(hiddenInstruction);
                  console.log(
                    "NEXIVRA ADAPTIVE GUIDANCE APPLIED:",
                    {prioritySkills,avoidOverTeaching}
                  );

                  const shouldPlanRolePlay =
                    !this.rolePlayActive &&
                    prioritySkills.some(
                      skill =>
                        skill.nextLearningAction === "create_natural_practice_opportunity" ||
                        skill.nextLearningAction === "provide_targeted_refresher"
                    );

                  if (shouldPlanRolePlay) {
                    this.requestAdaptiveRolePlay();
                  }
                }catch(error){
                  console.error("NEXIVRA ADAPTIVE GUIDANCE APPLY ERROR:",error);
                }
              }
