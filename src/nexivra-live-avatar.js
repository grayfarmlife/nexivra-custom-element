                ...
                guestMemoryUpdate:{
                  ...
                  guestOutcome:outcome.guestOutcome||''
                            ...
                            if(this.formalRolePlaySessionId){this.dispatchRuntimeEvent("nexivra-formal-role-play-complete",{rolePlaySessionId:this.formalRolePlaySessionId,outcomeSummary:outcome.outcomeSummary||"",memoryUpdate:{learnerKnownFacts:Array.isArray(outcome.learnerKnownFacts)?outcome.learnerKnownFacts:[],preferences:Array.isArray(outcome.preferences)?outcome.preferences:[],priorIssues:Array.isArray(outcome.priorIssues)?outcome.priorIssues:[],openCommitments:Array.isArray(outcome.openCommitments)?outcome.openCommitments:[],learnerCommitment:outcome.learnerCommitment||"",guestOutcome:outcome.guestOutcome||""}});}
                                  ...
                                  dispatchRuntimeEvent(
                                                          ...
                                                          startLearnerTranscriptCapture() {
                                                            ...
                                                            try{
                                                              ...
                                                              recognition.onresult=event=>{for(let i=event.resultIndex;i<event.results.length;i++){const result=event.results[i];if(!result?.isFinal)continue;const text=String(result[0]?.transcript||"").trim();if(!text)continue;const now=Date.now();if(text===this.lastLearnerTranscript&&now-this.lastLearnerTranscriptAt<5000)continue;this.lastLearnerTranscript=text;this.lastLearnerTranscriptAt=now;console.log("NEXIVRA LEARNER TRANSCRIPT CAPTURED:",text);const lowerText=String(text||"").toLowerCase();if(!this.rolePlayActive&&/\b(role[- ]?play|practice|redo|try)\b/.test(lowerText)){if(/\bsally\b/.test(lowerText))this.requestFormalRolePlayWithGuest("sally","Sally");else if(/\bron\b/.test(lowerText))this.requestFormalRolePlayWithGuest("ron","Ron");}if(this.rolePlayActive){this.rolePlayConversation.push({speaker:"learner",text,at:new Date().toISOString()});}if(this.rolePlayActive&&this.formalRolePlaySessionId){this.dispatchRuntimeEvent("nexivra-formal-role-play-turn",{rolePlaySessionId:this.formalRolePlaySessionId,speaker:"learner",text});}this.dispatchRuntimeEvent("nexivra-learner-transcript",{sessionId:this.runtimeSessionId||"",text,observation:this.observationTimeline.length?this.observationTimeline[this.observationTimeline.length-1]:null});}};
