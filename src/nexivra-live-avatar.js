                                                ...
                                                this.adaptiveRolePlayCooldownMs = 15000;
                                                            ...
                                                            this.eyeContactGuestFacingSamples = 0;
                                                            this.eyeContactAwaySamples = 0;
                                                            this.eyeContactLongestAwayMs = 0;
                                                            this.eyeContactAwayStartedAt = null;
                                                            this.eyeContactCurrentFacing = null;
                                                            this.eyeContactLastSummaryAt = 0;
                                                            this.eyeContactSummaryIntervalMs = 5000;
                    ...
                    EYE CONTACT COMPETENCY:
                    - Learner-facing name: Eye Contact.
                    - Internally, guest-facing face/head orientation is the proxy; never claim literal pupil gaze.
                    - Brief looks away are normal and should not be treated as negative performance.
                    - Use aggregated visual evidence across a role-play, never isolated frames.
                    - Eye Contact is contextual and developmental.
                    - Never infer respect, honesty, interest, emotion, disability, neurotype, culture, or intent from visual orientation.
                    - Visual evidence supports coaching and evaluation; it is not punitive scoring.
                - Visual summaries stay local during role-play and attach only to meaningful learner turns or the final role-play outcome; do not create continuous backend writes.
                    ...
                    - Learner-directed role-play preferences are allowed. Adapt the interaction style when requested while preserving the learning objective and evaluation fairness.
                        ...
                        this.resetEyeContactEvidence();

                            const known = Boolean(scenario.knownGuest);
                            const guestName = String(scenario.guestName || "").trim();

                            const briefing = known && guestName
                              ? `This is ${guestName}, ${scenario.guestRelationship || "a returning guest"} whom the learner already knows. The learner legitimately knows ${guestName}'s first name.`
                              : `This is ${scenario.guestRelationship || "a new guest"}. The learner has not met this guest before and does not know the guest's name at the start.`;
            ...
            visualEngagement:this.eyeContactSamples>=8?this.buildEyeContactSummary():null
                              }
                            );

                            this.emitEyeContactSummary();
                                  ...
                                  dispatchRuntimeEvent(
                                                          ...
                                                          startLearnerTranscriptCapture() {
                                                            ...
                                                            try{
                                                              ...
                                                              recognition.onresult=event=>{for(let i=event.resultIndex;i<event.results.length;i++){const result=event.results[i];if(!result?.isFinal)continue;const text=String(result[0]?.transcript||"").trim();if(!text)continue;const now=Date.now();if(text===this.lastLearnerTranscript&&now-this.lastLearnerTranscriptAt<5000)continue;this.lastLearnerTranscript=text;this.lastLearnerTranscriptAt=now;console.log("NEXIVRA LEARNER TRANSCRIPT CAPTURED:",text);this.dispatchRuntimeEvent("nexivra-learner-transcript",{sessionId:this.runtimeSessionId||"",text,observation:this.observationTimeline.length?this.observationTimeline[this.observationTimeline.length-1]:null,visualEngagement:this.rolePlayActive&&this.eyeContactSamples>=8?this.buildEyeContactSummary():null});}};
                                                          ...
                                                          resetEyeContactEvidence() {
                                                            ...
                                                            this.eyeContactAwayStartedAt=null;
                                                            this.eyeContactCurrentFacing=null;
                                                            this.eyeContactLastSummaryAt=Date.now();
                                                          }

                                                          recordEyeContactSample(guestFacing) {
                                                            if(!this.sessionActive||!this.rolePlayActive)return;
                                                            const now=Date.now();
                                                            this.eyeContactSamples+=1;
                                                            if(guestFacing){
                                                              this.eyeContactGuestFacingSamples+=1;
                                                              if(this.eyeContactCurrentFacing===false&&this.eyeContactAwayStartedAt){
                                                                this.eyeContactLongestAwayMs=Math.max(this.eyeContactLongestAwayMs,now-this.eyeContactAwayStartedAt);
                                                              }
                                                              this.eyeContactAwayStartedAt=null;
                                                              this.eyeContactCurrentFacing=true;
                                                            }else{
                                                              this.eyeContactAwaySamples+=1;
                                                              if(this.eyeContactCurrentFacing!==false)this.eyeContactAwayStartedAt=now;
                                                              this.eyeContactCurrentFacing=false;
                                                            }
                                                            if(now-this.eyeContactLastSummaryAt>=this.eyeContactSummaryIntervalMs)this.emitEyeContactSummary();
                                                          }

                                                          buildEyeContactSummary() {
                                                            const total=Math.max(1,this.eyeContactSamples);
                                                            return{
                                                              competencyId:"eye_contact",
                                                              competencyName:"Eye Contact",
                                                              measurement:"guest_facing_orientation_proxy",
                                                              sampleCount:this.eyeContactSamples,
                                                              guestFacingRatio:Number((this.eyeContactGuestFacingSamples/total).toFixed(3)),
                                                              awayRatio:Number((this.eyeContactAwaySamples/total).toFixed(3)),
                                                              longestAwayMs:this.eyeContactLongestAwayMs,
                                                              currentGuestFacing:this.eyeContactCurrentFacing,
                                                          ...
                                                          emitEyeContactSummary() {
                                                            if(this.eyeContactSamples<8)return null;
                                                            const summary=this.buildEyeContactSummary();
                                                            this.eyeContactLastSummaryAt=Date.now();
                                                            console.log("NEXIVRA EYE CONTACT SUMMARY:",summary);
                                                            return summary;
                                                          }
                                                          ...
                                                          evaluateLiveEvents(observation) {
                                                            ...
                                                            if(observation.faceDetected){
                                                              this.recordEyeContactSample(
                                                                Boolean(observation.faceData?.facingForward)
                                                              );
                                                            }
