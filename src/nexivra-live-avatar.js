            ...
            - If asked who you are, say: "I'm Elenora, your NEXIVRA instructor."`;
                  }
    ...
    Do NOT choose another resume point.
    Phase: ${cp.phase||"LEARNING"}
    Current objective: ${cp.objectiveId||""}
    Status: ${cp.objectiveStatus||"IN_PROGRESS"}
    Last completed: ${cp.lastCompletedObjective||""}
    Next objective: ${cp.nextObjective||""}
    Instructional position: ${cp.instructionalPosition||""}
    Recent summary: ${cp.sessionSummary||""}
    Give one brief natural recap, then continue exactly from this position.`;
            }

            // Migration/fallback path for learners whose history predates the
            // deterministic checkpoint collection.
            const hasLedgerHistory=Boolean(currentObjective||lastActivity||nextObjective);

            if(!firstSession&&hasLedgerHistory){
              console.log(
                "NEXIVRA RESUME SOURCE:",
                "RUNTIME_INSTRUCTIONAL_LEDGER"
              );

              return `SESSION_POSITION — AUTHORITATIVE
    This is a resumed learning session.
    The deterministic checkpoint table is not populated yet, so use the persisted NEXIVRA instructional ledger as the authoritative migration source.
    Do NOT restart the course.
    Do NOT repeat your original introduction.
    Do NOT present the opening course overview again.
    Current objective: ${currentObjective||"continue the current unfinished objective"}
    Last meaningful activity: ${lastActivity||"prior learning activity exists"}
    Next objective: ${nextObjective||"continue forward to the next unfinished objective"}
    Give one concise natural recap of the most recent relevant learning, then continue forward from this position.`;
            }

            if(!firstSession){
              console.log(
                "NEXIVRA RESUME SOURCE:",
                "RETURNING_SESSION_NO_LEDGER_POSITION"
              );

              return `SESSION_POSITION — AUTHORITATIVE
    This is a returning learning session.
    Do NOT introduce yourself again.
    Do NOT restart the course or present the original course introduction.
    No precise checkpoint is available yet. Continue from the most recent unfinished learning state supplied by NEXIVRA runtime context.`;
            }
