...
// src/nexivra-live-avatar.js
var NexivraLiveAvatar = class extends HTMLElement {
  ...
  bindControls() {
    ...
    const voiceButton = this.shadowRoot.getElementById(
      "voiceButton"
    );
    const cameraButton = this.shadowRoot.getElementById(
      "cameraButton"
    );
  ...
  setStatus(message) {
    ...
    const status = this.shadowRoot.getElementById(
      "status"
    );
    if (status) {
   ...
   */
  async startNexivra() {
    if (this._started || !this._sessionToken) {
      return;
    }
    this._started = true;
  ...
  async startVoice() {
    ...
    try {
      ...
      permissionStream.getTracks().forEach(
        (track) => track.stop()
      );
      this.setStatus(
      ...
      this.setStatus(
        "MIC ERROR: " + (error?.message || String(error))
      );
    }
  }
  /*
